...
struct Constant {
    ...
    static let YoutubeAPI_KEY = "AIzaSyByXZ1PbLriDsx-ujAWiYgn-LsesZPl6TE"
    static let YoutubeBASE_URL = "https://www.googleapis.com/youtube/v3"
}
...
class APICaller {
    ...
    func getDiscoverMovies(completion: @escaping (Result<[Title], Error>) -> Void) {
        guard let url = URL(string: "\(Constant.BASE_URL)/3/discover/movie?api_key=\(Constant.API_KEY)&language=en-US&page=1") else { return  }
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)) { data, _, error in
            guard let data = data, error == nil else { return }
    ...
    func search(with query: String, completion: @escaping (Result<[Title], Error>) -> Void) {
        ...
        guard let url = URL(string: "\(Constant.BASE_URL)/3/search/movie?api_key=\(Constant.API_KEY)&language=en-US&page=1&query=\(query)") else { return  }
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)) { data, _, error in
            guard let data = data, error == nil else { return }
    ...
    func getMovie(with query: String, completion: @escaping (Result<VideoElement, Error>) -> Void) {
        guard let query = query.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) else { return }
        guard let url = URL(string: "\(Constant.YoutubeBASE_URL)/search?q=\(query)&key=\(Constant.YoutubeAPI_KEY)") else { return  }
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)) { data, _, error in
            guard let data = data, error == nil else { return }
